(function(){
	...
	NODE = PATH.createSVGPathSegLinetoHorizontalAbs(1);
	
	// Building the test suite object
	testDefinition = {
		info : {
			title : 'SVGPathSegLinetoHorizontalAbs',
			comment : 'SVGPathSegLinetoHorizontalAbs corresponds to an "absolute horizontal lineto" (H) path data command.'
		},
		test : [
			function() {
				var out = {
				    	title: "Check the existance of the SVGPathSegLinetoHorizontalAbs object definition",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && typeof SVGPathSegLinetoHorizontalAbs != 'undefined' && NODE instanceof SVGPathSegLinetoHorizontalAbs && 1) || 0;
				} catch(e) {
			...
			function() {
				var out = {
				    	title: "SVGPathSegLinetoHorizontalAbs also implement SVGPathSeg",
				    	comment: "", 
				    	result: 0
				    }
