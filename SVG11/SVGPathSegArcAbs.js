(function(){
	...
	NODE = PATH.createSVGPathSegArcAbs(1,1,1,1,1,true,true);
	
	// Building the test suite object
	testDefinition = {
		info : {
			title : 'SVGPathSegArcAbs',
			comment : 'SVGPathSegArcAbs corresponds to an "absolute arcto" (A) path data command.'
		},
		test : [
			function() {
				var out = {
				    	title: "Check the existance of the SVGPathSegArcAbs object definition",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && typeof SVGPathSegArcAbs != 'undefined' && NODE instanceof SVGPathSegArcAbs && 1) || 0;
				} catch(e) {
			...
			function() {
				var out = {
				    	title: "SVGPathSegArcAbs also implement SVGPathSeg",
				    	comment: "", 
				    	result: 0
				    }
