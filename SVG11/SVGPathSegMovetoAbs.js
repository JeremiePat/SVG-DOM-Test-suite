(function(){
	...
	NODE = PATH.createSVGPathSegMovetoAbs(1,1);
	
	// Building the test suite object
	testDefinition = {
		info : {
			title : 'SVGPathSegMovetoAbs',
			comment : 'SVGPathSegMovetoAbs corresponds to an "absolute moveto" (M) path data command.'
		},
		test : [
			function() {
				var out = {
				    	title: "Check the existance of the SVGPathSegMovetoAbs object definition",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && typeof SVGPathSegMovetoAbs != 'undefined' && NODE instanceof SVGPathSegMovetoAbs && 1) || 0;
				} catch(e) {
			...
			function() {
				var out = {
				    	title: "SVGPathSegMovetoAbs also implement SVGPathSeg",
				    	comment: "", 
				    	result: 0
				    }
