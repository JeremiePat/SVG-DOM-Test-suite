(function(){
	...
	NODE = PATH.createSVGPathSegCurvetoQuadraticAbs(1,1,1,1);
	
	// Building the test suite object
	testDefinition = {
		info : {
			title : 'SVGPathSegCurvetoQuadraticAbs',
			comment : 'SVGPathSegCurvetoQuadraticAbs corresponds to an "absolute quadratic Bézier curveto" (Q) path data command.'
		},
		test : [
			function() {
				var out = {
				    	title: "Check the existance of the SVGPathSegCurvetoQuadraticAbs object definition",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && typeof SVGPathSegCurvetoQuadraticAbs != 'undefined' && NODE instanceof SVGPathSegCurvetoQuadraticAbs && 1) || 0;
				} catch(e) {
			...
			function() {
				var out = {
				    	title: "SVGPathSegCurvetoQuadraticAbs also implement SVGPathSeg",
				    	comment: "", 
				    	result: 0
				    }
			...
			function() {
				var out = {
				    	title: "The x1 attribute is of type Number",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && !isNaN(NODE.x1) && 1) || 0;
				} catch(e) {
					out.comment = e.message;
				}
			...
			function() {
				var out = {
				    	title: "The y1 attribute is of type Number",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && !isNaN(NODE.y1) && 1) || 0;
				} catch(e) {
					out.comment = e.message;
				}
