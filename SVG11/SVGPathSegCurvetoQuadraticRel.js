(function(){
	...
	NODE = PATH.createSVGPathSegCurvetoQuadraticRel(1,1,1,1);
	
	// Building the test suite object
	testDefinition = {
		info : {
			title : 'SVGPathSegCurvetoQuadraticRel',
			comment : 'SVGPathSegCurvetoQuadraticRel corresponds to an "relative quadratic Bézier curveto" (q) path data command.'
		},
		test : [
			function() {
				var out = {
				    	title: "Check the existance of the SVGPathSegCurvetoQuadraticRel object definition",
				    	comment: "", 
				    	result: 0
				    }
				
				try{
				    out.result = (NODE && typeof SVGPathSegCurvetoQuadraticRel != 'undefined' && NODE instanceof SVGPathSegCurvetoQuadraticRel && 1) || 0;
				} catch(e) {
			...
			function() {
				var out = {
				    	title: "SVGPathSegCurvetoQuadraticRel also implement SVGPathSeg",
				    	comment: "", 
				    	result: 0
				    }
